...
gen.compile(tf.train.GradientDescentOptimizer(learning_rate = 1), 'binary_crossentropy')
disc.compile(tf.train.GradientDescentOptimizer(learning_rate = 1), 'binary_crossentropy')
...
K = 200#Number of draws for each point
...
gen.compile(tf.train.GradientDescentOptimizer(learning_rate = 0.000001), 'binary_crossentropy')
disc.compile(tf.train.GradientDescentOptimizer(learning_rate = 0.000001), 'binary_crossentropy')
meanprob_true = []
meanprob_fake = []
epochs = 200000
#for epoch in tqdm(range(epochs)):
with tqdm(range(epochs)) as t:
    for epoch in t:
        ...
        meanprob_true = np.mean(prob_true)
        meanprob_fake = np.mean(prob_fake)
