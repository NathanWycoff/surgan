...
def permadrop(model_input):
    #return(tf.map_fn(lambda y: y if np.random.uniform() > drop_rate else tf.zeros_like(y), model_input))
    #return(tf.transpose(tf.map_fn(lambda y: tf.cond(tf.random.uniform(shape=[1]) > drop_rate, lambda: y, lambda: tf.zeros_like(y)), tf.transpose(model_input))))
    print(model_input)
    mask = tf.diag(tf.cast(tf.random.uniform(shape=[model_input.shape[1]])>drop_rate, tf.float32))
    return(tf.matmul(model_input, mask))

## Create our adversaries.
gen = tf.keras.Sequential()
gen.add(tf.keras.layers.Dense(H, input_dim = P + R, activation = tf.keras.activations.elu))
gen.add(tf.keras.layers.Lambda(permadrop))
