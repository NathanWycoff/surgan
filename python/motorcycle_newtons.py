...
for epoch in tqdm(range(epochs)):
    ...
    def disc_obj(weights, grad = False):
        ...
        if grad:
            return(np.concatenate([x.numpy().flatten() for x in grads]))
        else:
            return(total_cost.numpy())

    # Calculate Hessian
    nweights = len(np.concatenate([x.flatten() for x in disc.get_weights()]))
    disc.trainable = True
    with tf.GradientTape() as tt:
        ...
        grads = t.gradient(dl, disc.trainable_variables)
        flatgrads = tf.concat([tf.reshape(x,[np.prod(x.shape)]) for x in grads],0)
    tt.jacobian(flatgrads, disc.trainable_variables)
